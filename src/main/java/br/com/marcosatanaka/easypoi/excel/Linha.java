...
public class Linha {
  ...
  public void preencherCom(List<?> valores, CellStyle cellStyle) {
    valores.forEach(valor -> novaCelula().setCabecalho(valor, cellStyle));
  }
  ...
  protected List<Celula> celulas() {
    return StreamSupport.stream(row.spliterator(), false).map(cell -> Celula.of(cell, nomeColuna(cell)))
                        .collect(Collectors.toList());
  }

  public boolean possuiValores() {
    return StreamSupport.stream(row.spliterator(), false).map(Celula::of).anyMatch(Celula::possuiValor);
  }
